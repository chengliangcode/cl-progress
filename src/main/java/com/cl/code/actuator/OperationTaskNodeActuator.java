...
public abstract class OperationTaskNodeActuator<T extends NodeType<V>, V extends NodeProperty> extends TaskNodeActuator<T, V> {
    ...
    public Long operationTrigger(NodeDefinition<V> nodeDefinition, Flow flow, Long taskId, GrantedOperation grantedOperation, Map<String, Object> context) {
        // 是否支持该操作
        ...
        if (support) {
            ...
            if (executable) {
                TaskStatusEnum taskStatus = operationTrigger(nodeDefinition, flow, grantedOperation, context);
                // 更新 task 状态
     ...
     * @return {@code Long}
     */
    protected abstract TaskStatusEnum operationTrigger(NodeDefinition<V> nodeDefinition, Flow flow, GrantedOperation grantedOperation, Map<String, Object> context);
